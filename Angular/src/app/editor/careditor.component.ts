...
import { Cararticle, CararticlesService } from '../shared';

@Component({
  selector: 'careditor-page',
...
export class CareditorComponent implements OnInit {
  ...
  constructor(
    ...
    this.cararticleForm = this.fb.group({
      title: '',
      description: '',
      location: '',
      landmark: '',
  ...
  ngOnInit() {
    ...
    this.route.data.subscribe(
      (data: {cararticle: Cararticle}) => {
        if (data.cararticle) {
          this.cararticle = data.cararticle;
          this.cararticleForm.patchValue(data.cararticle);
        }
      }
    );
  }
  ...
  addTag() {
    ...
    if (this.cararticle.tagList.indexOf(tag) < 0) {
      this.cararticle.tagList.push(tag);
    }
    // clear the input
    this.tagField.reset('');
  }
  ...
  removeTag(tagName: string) {
    this.cararticle.tagList = this.cararticle.tagList.filter((tag) => tag !== tagName);
  }
  ...
  submitForm() {
    ...
    // update the model
    this.updateArticle(this.cararticleForm.value);

    // post the changes
    this.cararticlesService
    .save(this.cararticle)
    .subscribe(
  ...
  updateArticle(values: Object) {
    (<any>Object).assign(this.cararticle, values);
  }
