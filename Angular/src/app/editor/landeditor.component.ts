...
import { Landarticle, LandarticlesService } from '../shared';

@Component({
  selector: 'landeditor-page',
  templateUrl: './landeditor.component.html'
})
export class LandeditorComponent implements OnInit {
  imageId: string;
  imageId2: string;
  imageId3: string;
  //cloudinaryImage: any;
  ...
  landarticle: Landarticle = new Landarticle();
  landarticleForm: FormGroup;
  ...
  constructor(
    ...
    this.landarticleForm = this.fb.group({
      title: '',
      description: '',
      location: '',
      phone: '',
  ...
  ngOnInit() {
    ...
    this.route.data.subscribe(
      (data: {landarticle: Landarticle}) => {
        if (data.landarticle) {
          this.landarticle = data.landarticle;
          this.landarticleForm.patchValue(data.landarticle);
        }
      }
    );
  }
  ...
  addTag() {
    ...
    if (this.landarticle.tagList.indexOf(tag) < 0) {
      this.landarticle.tagList.push(tag);
    }
    // clear the input
    this.tagField.reset('');
  }
  ...
  removeTag(tagName: string) {
    this.landarticle.tagList = this.landarticle.tagList.filter((tag) => tag !== tagName);
  }
  ...
  submitForm() {
    ...
    // update the model
    this.updateArticle(this.landarticleForm.value);

    // post the changes
    this.landarticlesService
    .save(this.landarticle)
    .subscribe(
  ...
  updateArticle(values: Object) {
    (<any>Object).assign(this.landarticle, values);
  }
