...
import { Apatarticle, ApatarticlesService } from '../shared';

@Component({
  selector: 'apateditor-page',
  templateUrl: './apateditor.component.html'
})
export class ApateditorComponent implements OnInit {
  imageId: string;
  imageId2: string;
  imageId3: string;
  //cloudinaryImage: any;
  ...
  apatarticle: Apatarticle = new Apatarticle();
  apatarticleForm: FormGroup;
  ...
  constructor(
    ...
    this.apatarticleForm = this.fb.group({
      title: '',
      description: '',
      location: '',
      phone: '',
  ...
  ngOnInit() {
    ...
    this.route.data.subscribe(
      (data: {apatarticle: Apatarticle}) => {
        if (data.apatarticle) {
          this.apatarticle = data.apatarticle;
          this.apatarticleForm.patchValue(data.apatarticle);
        }
      }
    );
  }
  ...
  addTag() {
    ...
    if (this.apatarticle.tagList.indexOf(tag) < 0) {
      this.apatarticle.tagList.push(tag);
    }
    // clear the input
    this.tagField.reset('');
  }
  ...
  removeTag(tagName: string) {
    this.apatarticle.tagList = this.apatarticle.tagList.filter((tag) => tag !== tagName);
  }
  ...
  submitForm() {
    ...
    // update the model
    this.updateArticle(this.apatarticleForm.value);

    // post the changes
    this.apatarticlesService
    .save(this.apatarticle)
    .subscribe(
  ...
  updateArticle(values: Object) {
    (<any>Object).assign(this.apatarticle, values);
  }
